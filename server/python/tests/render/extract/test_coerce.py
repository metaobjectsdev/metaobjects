...
def _normal() -> ExtractOptions:
    return ExtractOptions.defaults()


def _kinds(rep: ExtractionReport) -> list[str]:
...
def test_runtime_alias_wins_over_schema() -> None:
    ...
    opts = ExtractOptions(tolerance=Tolerance.NORMAL, aliases={"x": "HOSTILE"})
...
def test_on_field_hook_wins() -> None:
    ...
    opts = ExtractOptions(on_field=lambda path, raw, spec: "HOOKED")
...
def test_normalizer_by_field_name_applied() -> None:
    rep = ExtractionReport()
    f = FieldSpec.scalar("x", FieldKind.STRING, True)
    opts = ExtractOptions(normalizers={"x": lambda raw: raw.upper()})
...
def test_enum_normalize_strip_extracts_punctuated_variant() -> None:
    rep = ExtractionReport()
...
def test_enum_normalize_collapse_extracts_spaced_variant() -> None:
    rep = ExtractionReport()
