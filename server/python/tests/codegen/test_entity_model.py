...
def _value(name: str, fields: list[MetaField], *, package: str | None = None) -> MetaObject:
    o = MetaObject(TYPE_OBJECT, "value", name)
    o.package = package
    for f in fields:
        o.add_child(f)
    return o
...
def test_vanilla_entity_model() -> None:
    ...
    # #224 — FR-036 Pin 1 (implicit min_length=1 on a @required string) is WIRE-TIER
    # ONLY: the READ model carries no implicit floor (other constraints, like
    # @maxLength, still apply); the WIRE model (SubscriberCreate) keeps Pin 1.
    read_model = out.split("class SubscriberCreate")[0]
    assert "email: str = Field(max_length=200)" in read_model
    assert "min_length" not in read_model
    assert "subscribed: bool | None = None" in read_model
...
# --------------------------------------------------------------------------- #
# #224 — FR-036 Pin 1 (@required non-array string ⇒ non-empty) is a WIRE-TIER
# rule, not an in-process one. A downstream adopter's in-process prompt-payload
# object.value models broke on legitimately-empty slots because the read model
# self-validated at construction. The fix scopes Pin 1 to the wire (Create/Patch)
# models; the in-process read model (entity OR value) carries no implicit floor,
# and a nested object.value keeps wire-tier enforcement via its own <Name>Create.
# --------------------------------------------------------------------------- #


def test_value_object_required_string_accepts_empty_in_process() -> None:
    """The adopter's exact repro: an in-process object.value with a @required
    string must construct cleanly with "" — Pin 1 does not apply in-process."""
    e = _value("Slots", [_f("goal", fc.FIELD_SUBTYPE_STRING, required=True)])
    mod = _load_models(e)
    assert mod.Slots(goal="").goal == ""


def test_value_object_wire_create_model_still_rejects_empty_required_string() -> None:
    """Tier separation, direction 2 — the VO's own WIRE model (<Name>Create) keeps
    Pin 1: a required string still rejects "" there, so nesting a VO into an
    entity's wire body (api-contract jsonb r10) stays enforced."""
    e = _value("Slots", [_f("goal", fc.FIELD_SUBTYPE_STRING, required=True)])
    mod = _load_models(e)
    with pytest.raises(ValidationError):
        mod.SlotsCreate(goal="")


def test_entity_read_model_no_longer_carries_implicit_min_length() -> None:
    """Issue §4 (same fix, zero marginal cost) — the ENTITY read model also loses
    the implicit min_length=1: reading an existing row with "" must not throw.
    The entity's wire Create model still enforces it (proven elsewhere)."""
    e = _entity("Widget", [_f("name", fc.FIELD_SUBTYPE_STRING, required=True)], package="app::w")
    mod = _load_models(e)
    assert mod.Widget(name="").name == ""
    with pytest.raises(ValidationError):
        mod.WidgetCreate(name="")
