...
def test_gen_baseline_adopt_records_without_writing(tmp_path: Path, capsys) -> None:
    """`--baseline=adopt` reaches the write path — the flag's whole value is threading.

    A pre-manifest project: output on disk that no longer matches fresh output, and no
    `.gen-state`. A plain `gen` refuses it and exits non-zero; `--baseline=adopt` records
    it and writes nothing, so the manifest the refusal asks the adopter to commit finally
    exists. The parser accepting the flag proves nothing on its own — the defect this
    guards against is a flag that parses and is then dropped before `GenConfig`.
    """
    meta_dir = _meta_dir(tmp_path)
    out = tmp_path / "out"
    assert main(["gen", meta_dir, "--out", str(out)]) == 0
    ...
    assert "as the codegen baseline" in capsys.readouterr().out


def test_gen_exits_non_zero_when_a_file_is_refused(tmp_path: Path) -> None:
    """A refusal is a FAILED gate, exactly as it is on TypeScript.

    Python and C# used to print the warning and exit 0, so an adopter who wired
    `metaobjects gen` into CI as a drift gate was green while codegen was refusing to
    write — the gate silently did not gate. The exit code is the only part of this a CI
    step reads.
    """
    meta_dir = _meta_dir(tmp_path)
    out = tmp_path / "out"
