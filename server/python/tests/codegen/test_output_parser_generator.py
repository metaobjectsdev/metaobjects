...
def test_render_imports_response_and_emits_parse_function() -> None:
    """Parser module imports the ``@responseRef`` value object's own model from the
    entity generator's module (ADR-0056) and exposes a throw-only ``parse_<name>(text)``
    entry point."""
    root = _npc_root()
    out = render_output_parser(_template(root, "NpcResponsePrompt"), root)
    assert out is not None
    # The strict type is the value object's model — never declared here. (FR-010 adds a
    # separate nullable mirror dataclass for extract() — see the extract tests below —
    # but the model is never inlined.)
    assert "class NpcAnswer(" not in out
    assert "from .NpcAnswer import NpcAnswer" in out
    assert "def parse_npc_response_prompt(text: str) -> NpcAnswer:" in out
    assert "return NpcAnswer.model_validate_json(text)" in out
    # The mirror is keyed by the value object, not the template (ADR-0056 rule 3).
    assert "class NpcAnswerExtracted:" in out
    assert "NpcResponsePromptResponse" not in out
    # ADR-0052 — the REQUEST shape is never referenced by the parser.
    assert "NpcBrief" not in out
...
def test_render_includes_generated_header_no_pydantic_import() -> None:
    ...
    value object's model module, not the parser module."""
    root = _npc_root()
    out = render_output_parser(_template(root, "NpcResponsePrompt"), root)
    assert out is not None
    assert "@generated by metaobjects" in out
    # Pydantic lives in the model module.
...
def test_xml_reply_emits_no_strict_parser_but_keeps_the_tolerant_extract() -> None:
    ...
    # ...and the model is not imported either, which would be a dead import.
    assert "from .NpcAnswer import" not in out
