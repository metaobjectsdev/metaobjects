...
def render_trace_helper(entity: MetaObject, root: MetaData) -> str | None:
    ...
    lines.extend(
        [
            ...
            "    # A required field lost OR malformed (present but unusable) has no value,",
            "    # so either fails the call.",
            "    unusable = []",
            "    if outcome.report.has_lost_required():",
            '        unusable.append("lost required: " + ", ".join(outcome.report.lost_required()))',
            "    if outcome.report.has_malformed_required():",
            "        unusable.append(",
            '            "malformed required: " + ", ".join(outcome.report.malformed_required())',
            "        )",
            "    failed = bool(unusable)",
            "    status = STATUS_ERROR if failed else STATUS_OK",
            '    error_detail = "; ".join(unusable) if failed else None',
