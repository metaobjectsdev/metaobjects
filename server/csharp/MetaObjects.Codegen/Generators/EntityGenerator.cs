...
public class EntityGenerator : IGenerator
{
    ...
    public virtual IEnumerable<EmittedFile> Generate(GenContext ctx)
    {
        ...
        // ADR-0056 — every value object gets its POCO here, and this POCO is THE C# type for
        // it: the template tier (render helpers, parsers, extractors) references it and
        // declares none of its own. So the set is every concrete object.value and every
        // concrete sourceless projection, whether or not an entity reaches it, plus any
        // abstract value object an entity object-field does reach (its owned-type navigation
        // needs the class).
        var valueObjects = ValueShapes(ReferencedValueObjects([.. mapped, .. unmappedShapes], ctx), ctx);
    ...
    {
        // ADR-0056: the ONE name every generator uses for this type (collision-qualified when
        // two value objects share a short name).
        var className = ValueObjectNames.TypeName(vo, ctx.Root);
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
    ...
    {
        if (ValueObjectNames.ResolveFieldRef(field, ctx.Root) is not { } target)
        {
            ctx.Warn($"{Name}: object-typed field \"{owner.Name}.{field.Name}\" has an unresolved @objectRef \"{field.ObjectRef}\" — skipped.");
            return null;
        }
        var typeName = ValueObjectNames.TypeName(target, ctx.Root);
        var propName = PropertyName(field);
        var required = CSharpNaming.IsRequired(owner, field);
    ...
    {
        var valueType = CSharpNaming.MapValueType(field, ctx.Root);
        var propName = PropertyName(field);
        var required = CSharpNaming.IsRequired(owner, field);
    ...
    protected static List<MetaObject> ReferencedValueObjects(IReadOnlyList<MetaObject> mapped, GenContext ctx)
    {
        ...
        foreach (var e in mapped.Where(o => o.IsEntity() && !o.IsReadOnlyProjection()))
            foreach (var f in e.Fields().Where(ReferencesValueObject))
                Enqueue(ValueObjectNames.ResolveFieldRef(f, ctx.Root));
        ...
        foreach (var e in mapped)
            foreach (var src in e.Sources())
                if (src.ParameterRef is { } pref)
                    Enqueue(global::MetaObjects.NamingRefs.ResolveObjectRef(
                        ctx.Root, pref, global::MetaObjects.NamingRefs.EffectivePackage(e)) as MetaObject);
        ...
        void Enqueue(MetaObject? target)
        {
            // Only plain value objects (no source) become POCOs; entities/projections
            // are already in `mapped`. Deduped by FQN: two value objects may share a short name.
            if (target is null || !target.IsValue() || target.DbView is not null) return;
            if (seen.Add(target.ResolutionKey())) queue.Enqueue(target);
        }
    }

    // ADR-0056 — the value shapes this run emits a POCO for: the reachable closure above, plus
    // every concrete object.value and concrete sourceless projection in the model. Deduped by
    // FQN and ordered by emitted name, so the output order is a pure function of the model.
    protected static List<MetaObject> ValueShapes(IReadOnlyList<MetaObject> referenced, GenContext ctx)
    {
        var byFqn = new Dictionary<string, MetaObject>(StringComparer.Ordinal);
        foreach (var vo in referenced)
            byFqn.TryAdd(vo.ResolutionKey(), vo);
        foreach (var o in ctx.Entities.Where(o => !o.IsAbstract && ValueObjectNames.IsValueShape(o)))
            byFqn.TryAdd(o.ResolutionKey(), o);
        return byFqn.Values
            .OrderBy(o => ValueObjectNames.TypeName(o, ctx.Root), StringComparer.Ordinal)
            .ToList();
    }
}
