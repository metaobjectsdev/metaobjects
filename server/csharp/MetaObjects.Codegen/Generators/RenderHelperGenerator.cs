...
public class RenderHelperGenerator : IGenerator
{
    ...
    protected virtual EmittedFile EmitHelper(MetaData tmpl, MetaData vo, string payloadRef, GenContext ctx)
    {
        ...
        var kind = (tmpl.OwnAttr(TEMPLATE_ATTR_KIND) as string ?? TEMPLATE_KIND_DEFAULT).ToLowerInvariant();
        ...
        {
            var subjectRef = tmpl.OwnAttr(TEMPLATE_ATTR_SUBJECT_REF) as string;
            var htmlBodyRef = tmpl.OwnAttr(TEMPLATE_ATTR_HTML_BODY_REF) as string;
            var textBodyRef = tmpl.OwnAttr(TEMPLATE_ATTR_TEXT_BODY_REF) as string;
        ...
        else
        {
            ...
            var format = tmpl.OwnAttr(TEMPLATE_ATTR_FORMAT) as string ?? TEMPLATE_FORMAT_DEFAULT;
        ...
        MetaRoot root, MetaData vo, HashSet<string> seen)
    {
        ...
        foreach (var f in vo.Children().Where(c => c.Type == TYPE_FIELD))
        {
            if (f.SubType == FIELD_SUBTYPE_OBJECT &&
                f.OwnAttr(FIELD_ATTR_OBJECT_REF) is string refName)
            {
    ...
    private static MetaData? ResolveNestedObjectRef(MetaRoot root, string reference)
    {
        ...
        return root.OwnChildren().FirstOrDefault(c =>
    ...
    /// <summary>Resolve @maxChars (own attr) as an int, or null when absent/blank/non-numeric.</summary>
    private static int? MaxCharsOf(MetaData tmpl)
    {
        var v = tmpl.OwnAttr(TEMPLATE_ATTR_MAX_CHARS);
        return v switch
        {
            int i => i,
            long l => (int)l,
