...
public sealed class OutputParserGenerator : IGenerator
{
    ...
    private static EmittedFile EmitParser(MetaData tmpl, string payloadRef, GenContext ctx)
    {
        ...
        var recoveredType = $"{payloadType}Recovered";

        // FR-010: emit the tolerant recover() API alongside strict Parse/TryParse when the
        // template targets json/xml AND the @payloadRef resolves to a value-object we can
        // bake a RecoverSchema from. Otherwise only the FR-006 strict parser is emitted.
        ...
        bool emitRecover = formatSupportsRecover && vo is not null;
        ...
        if (emitRecover) sb.AppendLine("using MetaObjects.Render.Recover;");
        sb.AppendLine();
        ...
        if (emitRecover)
        {
            ...
            string initializer = RecoverSchemaEmitter.MirrorInitializer(vo!, recoveredType);
            sb.AppendLine();
            sb.AppendLine("    // FR-010 tolerant recovery — best-effort, never throws. Returns a nullable");
            ...
            sb.AppendLine($"        return new RecoveryResult<{recoveredType}>(data, o.Report);");
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine("    /// <summary>Recovery as a bool gate: <c>true</c> when the response was non-empty and no required field was lost.</summary>");
            sb.AppendLine($"    public static bool TryRecover(string text, out RecoveryResult<{recoveredType}> result)");
            sb.AppendLine("    {");
