...
public static class PostgresEmit
{
    ...
    private static int StageOf(Change c) => c switch
    {
        ...
        Change.AddColumn or Change.DropColumn or Change.ChangeColumnType
            or Change.ChangeColumnNullable or Change.ChangeColumnDefault
            or Change.ChangeColumnIdentity => 2,
    ...
    private static string RenderUp(Change c) => c switch
    {
        ...
        Change.ChangeColumnIdentity x => RenderIdentityChange(x.Table, x.Schema, x.Column, x.To),
    ...
    private static string RenderDown(Change c) => c switch
    {
        ...
        Change.ChangeColumnDefault x => x.From is not null
            ...
            : $"ALTER TABLE {QualifiedTable(x.Table, x.Schema)} ALTER COLUMN {Quote(x.Column)} DROP DEFAULT;",
        Change.ChangeColumnIdentity x => RenderIdentityChange(x.Table, x.Schema, x.Column, x.From),
    ...
    // Identity attach/detach. Postgres syntax is ADD GENERATED ... AS IDENTITY for
    // attach and DROP IDENTITY IF EXISTS for detach (the IF EXISTS makes the DDL
    // safely idempotent on a no-op replay). Uuid identity rides through column
    // defaults (gen_random_uuid()), so the only kind we render here is Increment;
    // a null `to` means "remove identity from this column".
    private static string RenderIdentityChange(string table, string? schema, string column, IdentityKind? to)
    {
        var qual = $"ALTER TABLE {QualifiedTable(table, schema)} ALTER COLUMN {Quote(column)}";
        return to switch
        {
            IdentityKind.Increment => $"{qual} ADD GENERATED BY DEFAULT AS IDENTITY;",
            null => $"{qual} DROP IDENTITY IF EXISTS;",
            _ => throw new InvalidOperationException($"unhandled identity kind: {to}"),
        };
    }
