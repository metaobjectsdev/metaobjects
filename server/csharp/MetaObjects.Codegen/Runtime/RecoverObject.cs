...
//   • the recover ENGINE (MetaObjects.Render.Recover: RecoverSchema / FieldSpec / Recover) —
//     parses dirty JSON/XML into a forgiving Dictionary/List tree + a RecoveryReport;
...
//
// CONTRACT. Recover(...) NEVER throws: lost/malformed fields are classified in the report,
// never raised. Opt into strictness with RecoveryResult<T>.OrThrow() (in Render), which throws
// a RecoverException iff a required field was lost.
//
...
/// <summary>
/// Runtime bridge that recovers dirty model output into a populated, typed object graph
/// described by a <see cref="MetaObject"/>. Never throws; <see cref="RecoveryResult{T}.OrThrow"/>
/// is the opt-in strict gate.
/// </summary>
public static class RecoverObject
{
    /// <summary>
    ...
    /// <summary>
    /// Recover <paramref name="text"/> into a typed object graph described by <paramref name="mo"/>.
    ///
    /// <para>Pipeline: build a <see cref="RecoverSchema"/> from <paramref name="mo"/> (driven
    ...
    public static RecoveryResult<object> Recover(
        MetaObject mo,
        string? text,
        Format format = Format.Json,
        RecoverOptions? opts = null)
    {
        RecoverSchema schema = RecoverSchemaFor(mo, format);
        RecoverOutcome outcome = Render.Recover.Recover.Run(text, schema, opts ?? RecoverOptions.Defaults());
        object obj = Assemble(mo, outcome.Data);
        return new RecoveryResult<object>(obj, outcome.Report);
    }
    ...
    /// <summary>
    /// Build a <see cref="RecoverSchema"/> for <paramref name="mo"/> by walking its
    ...
    /// </summary>
    public static RecoverSchema RecoverSchemaFor(MetaObject mo, Format format = Format.Json) =>
        RecoverSchemaFor(mo, format, NewVisited(), 0);
        ...
        HashSet<MetaObject> visited, int depth)
    {
        ...
        return new RecoverSchema(format, mo.Name, fields);
    }
        ...
        HashSet<MetaObject> visited, int depth)
    {
        ...
        if (field.SubType == FIELD_SUBTYPE_OBJECT || field.ObjectRef is not null)
        {
            ...
            RecoverSchema nested = RecoverSchemaFor(referenced!, format, visited, depth + 1);
    ...
    // =========================================================================
    // Assemble — recovered Dictionary/List tree -> object graph
    // =========================================================================

    /// <summary>
    /// Assemble a recovered forgiving tree into a populated object graph described by
    ...
    /// <see cref="RecoverSchemaFor(MetaObject, Format)"/>. Reflection-free; never throws on
    ...
    // =========================================================================
    // Enum-attr reading (mirrors RecoverSchemaEmitter / Fr010FieldMapping)
    // =========================================================================
