# Java recover engine — known gaps & intentional cross-port divergences

Scope: the tolerant `recover` pipeline (`com.metaobjects.render.recover`). This Java engine is the
...
- **Nested/embedded-object recovery is now supported** uniformly at the engine level (dotted child
  ...
  nested recovery is reachable through a hand-built / engine-level schema but is not yet auto-emitted
  by codegen.
...
- **Numeric suffix / hex-float literals.** Java's `Double.parseDouble` accepts `"42d"` / `"42f"`
  and hex-float forms (→ RECOVERED); the data-oriented ports reject them → MALFORMED. The
