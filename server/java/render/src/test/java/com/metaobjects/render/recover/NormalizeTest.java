...
public class NormalizeTest {
    ...
    private static Object coerceEnum(String raw, FieldSpec spec, RecoverOptions opts) {
        return Coerce.value(raw, spec, opts, spec.name(), new RecoveryReport());
    }
    ...
    public void exactMatchWins() {
        ...
        assertEquals("DONE", coerceEnum("DONE", spec, RecoverOptions.defaults()));
    }

    @Test
    public void normalizeStripFoldsOffVocab() {
        FieldSpec spec = enumSpec(Normalize.STRIP, null, Map.of());
        assertEquals("IN_PROGRESS", coerceEnum("in progress", spec, RecoverOptions.defaults()));
    }
    ...
    public void normalizeCollapseRejectsRunTogether() {
        ...
        assertEquals(Coerce.MALFORMED, coerceEnum("inprogress", spec, RecoverOptions.defaults()));
    }

    @Test
    public void normalizeNoneRejectsAnyVariation() {
        FieldSpec spec = enumSpec(Normalize.NONE, null, Map.of());
        assertEquals(Coerce.MALFORMED, coerceEnum("in progress", spec, RecoverOptions.defaults()));
        assertEquals("DONE", coerceEnum("DONE", spec, RecoverOptions.defaults()));
    }
    ...
    public void coerceDefaultIsTerminalFallback() {
        ...
        assertEquals("DONE", coerceEnum("banana", spec, RecoverOptions.defaults()));
    }

    @Test
    public void schemaAliasResolvesBeforeCoerceDefault() {
        FieldSpec spec = enumSpec(Normalize.NONE, "DONE", Map.of("wip", "IN_PROGRESS"));
        assertEquals("IN_PROGRESS", coerceEnum("wip", spec, RecoverOptions.defaults()));
    }
    ...
    public void aliasKeyMatchesUnderNormalizationMode() {
        ...
        assertEquals("IN_PROGRESS", coerceEnum("w.i.p", spec, RecoverOptions.defaults()));
    }

    @Test
    public void strictToleranceForcesExactOnly() {
        FieldSpec spec = enumSpec(Normalize.STRIP, "DONE", Map.of());
        RecoverOptions strict = RecoverOptions.defaults().withTolerance(Tolerance.STRICT);
