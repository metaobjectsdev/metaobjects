...
public object KotlinGenUtil {
            ...
            if (existing != resolved) {
                throw GeneratorException(
                    "${obj.name} declares more than one source.rdb with @role: \"$role\", and " +
                        "they disagree on the object's physical address: $existing vs " +
                        "$resolved. The names artifact keys " +
    ...
    /**
     * True when [field] is SERVER-OWNED on create for [obj]: a component of [obj]'s
     * `increment`/`uuid` primary key (the store generates it), or an `@autoSet` column (the
     * CRUD path stamps it, issue #203). The caller never supplies either, so a create body
     * that omits one is correct — the complement of [isAssignedPrimaryKeyField]. Judged
     * against the entity being generated, not `field.parent`: a key idiomatically lives on
     * an abstract base and is inherited through `extends`.
     *
     * Mirrors the Java port's `SERVER_OWNED_ON_CREATE` set (SpringControllerGenerator).
     */
    fun isServerOwnedOnCreate(field: MetaField<*>, obj: MetaObject): Boolean {
        val pk = obj.primaryIdentity
        if (pk != null && pk.isAutoGenerated && pk.fields.contains(field.name)) return true
        return autoSetPolicy(field) != null
    }

    /**
     * True when [field] is a `field.enum` stored as its declared INTEGER (`@intValueMap`) rather
     * than its member symbol. Read resolving (ADR-0039) — the same test the Exposed table
     * generator applies when it emits the INTEGER `customEnumeration` column, so the column and
     * the filter that reads it cannot disagree.
     */
