...
/**
 * Unit tests for [KotlinExtractSchemaEmitter] — FR-010 Kotlin port of the
 * VO-to-extract-schema source-string helper.
 *
 ...
 *
 * Mirrors the Java `ExtractSchemaEmitterTest` contract adapted for Kotlin collection
...
class KotlinExtractSchemaEmitterTest {
    ...
    @Test fun schemaLiteralEmitsExtractSchemaWithJsonFormat() {
        val s = KotlinExtractSchemaEmitter.schemaLiteral(vo(), "json", "AnswerOutputPayload")

        assertTrue(
            "ExtractSchema(Format.JSON, \"AnswerOutputPayload\"" in s,
            "expected ExtractSchema(Format.JSON, \"AnswerOutputPayload\"; saw:\n$s"
        )
    }
    ...
    @Test fun schemaLiteralXmlFormatProducesFormatXml() {
        val s = KotlinExtractSchemaEmitter.schemaLiteral(vo(), "xml", "AnswerOutputPayload")

        assertTrue(
            "ExtractSchema(Format.XML, \"AnswerOutputPayload\"" in s,
            "expected Format.XML for xml format; saw:\n$s"
        )
    }

    // -------------------------------------------------------------------------
    // extractedClassDecl tests
    // -------------------------------------------------------------------------

    @Test fun extractedClassDeclEmitsDataClass() {
        val s = KotlinExtractSchemaEmitter.extractedClassDecl(vo(), "AnswerOutputExtracted")

        assertTrue(
            "data class AnswerOutputExtracted(" in s,
            "expected data class declaration; saw:\n$s"
        )
    }

    @Test fun extractedClassDeclTextIsNullableString() {
        val s = KotlinExtractSchemaEmitter.extractedClassDecl(vo(), "AnswerOutputExtracted")
    ...
    @Test fun extractedClassDeclConfidenceIsNullableString() {
        val s = KotlinExtractSchemaEmitter.extractedClassDecl(vo(), "AnswerOutputExtracted")
    ...
    @Test fun extractedClassDeclNoteIsNullableString() {
        val s = KotlinExtractSchemaEmitter.extractedClassDecl(vo(), "AnswerOutputExtracted")
    ...
    // -------------------------------------------------------------------------
    // extractedCtorArgs tests
    // -------------------------------------------------------------------------

    @Test fun extractedCtorArgsUsesAsStringForText() {
        val a = KotlinExtractSchemaEmitter.extractedCtorArgs(vo())

        assertTrue(
            "ExtractMap.asString(d, \"text\")" in a,
            "expected asString for text; saw:\n$a"
        )
    }

    @Test fun extractedCtorArgsUsesAsStringForConfidence() {
        val a = KotlinExtractSchemaEmitter.extractedCtorArgs(vo())

        assertTrue(
            "ExtractMap.asString(d, \"confidence\")" in a,
            "expected asString for confidence (enum-backed); saw:\n$a"
        )
    }

    @Test fun extractedCtorArgsUsesAsStringForNote() {
        val a = KotlinExtractSchemaEmitter.extractedCtorArgs(vo())

        assertTrue(
            "ExtractMap.asString(d, \"note\")" in a,
    ...
    @Test fun schemaLiteralIntegerFieldMapsToIntKind() {
        ...
        val s = KotlinExtractSchemaEmitter.schemaLiteral(numVo, "json", "NumericPayload")
    ...
    @Test fun extractedCtorArgsNumericTypes() {
        val fx = """
            {
              "metadata.root": { "package": "acme::types", "children": [
                { "object.value": { "name": "NumericPayload", "children": [
        ...
        val a = KotlinExtractSchemaEmitter.extractedCtorArgs(numVo)

        assertTrue("ExtractMap.asInt(d, \"count\")"    in a, "expected asInt; saw:\n$a")
        assertTrue("ExtractMap.asLong(d, \"totalMs\")" in a, "expected asLong; saw:\n$a")
        assertTrue("ExtractMap.asDouble(d, \"score\")" in a, "expected asDouble; saw:\n$a")
        assertTrue("ExtractMap.asBool(d, \"valid\")"   in a, "expected asBool; saw:\n$a")
    }
    ...
    @Test fun enumWithNoAliasEmitsEmptyMapOf() {
        ...
        val s = KotlinExtractSchemaEmitter.schemaLiteral(simpleVo, "json", "SimpleEnum")
    ...
    @Test fun schemaLiteralEmitsFr011CoerceDefaultAndResolvedNormalize() {
        ...
        val s = KotlinExtractSchemaEmitter.schemaLiteral(voFr, "json", "Fr011Payload")
    ...
    // -------------------------------------------------------------------------
    // isArray + EnumField: extractedClassDecl / extractedCtorArgs consistency
    // -------------------------------------------------------------------------
    ...
    @Test fun isArrayEnumFieldUsesListStringTypeAndAsStringList() {
        ...
        // constructorArgForField must return ExtractMap.asStringList(...).
        ...
        val decl = KotlinExtractSchemaEmitter.extractedClassDecl(tagVo, "TagPayloadExtracted")
        assertTrue(
            "val tags: List<String>? = null" in decl,
            "isArray enum field must use List<String>? type; saw:\n$decl"
        )

        val args = KotlinExtractSchemaEmitter.extractedCtorArgs(tagVo)
        assertTrue(
            "ExtractMap.asStringList(d, \"tags\")" in args,
            "isArray enum field must use asStringList, not asString; saw:\n$args"
        )
        assertTrue(
            "ExtractMap.asString(d, \"tags\")" !in args,
