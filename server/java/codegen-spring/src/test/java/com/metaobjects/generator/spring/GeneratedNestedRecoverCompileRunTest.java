 ...
 * {@code recover(MetaDataLoader, String)} on DIRTY nested JSON. Asserts the nested
 * object AND the array-of-objects components are fully populated (NOT null) in the
 * typed payload — which the self-contained {@code recover(String)} cannot do.
 *
 ...
 * {@link com.metaobjects.object.recover.MetaObjectRecover}, then maps the assembled
 * {@code ValueObject} graph into the typed-record graph via generated mappers.
 */
public class GeneratedNestedRecoverCompileRunTest extends SharedRegistryTestBase {

    @Rule
    public TemporaryFolder tmp = new TemporaryFolder();

    @Test
    public void delegatingRecoverPopulatesNestedObjectAndArrayOfObjects() throws Exception {
        // -----------------------------------------------------------------------
        ...
        MetaDataLoader loader = SpringTestFixtures.loadFixture(
                ws, "recover-nested-cr", SpringTestFixtures.RECOVER_NESTED_FIXTURE);
        ...
        // -----------------------------------------------------------------------
        // 4. Load the parser, invoke recover(MetaDataLoader, String) on dirty nested JSON
        // -----------------------------------------------------------------------
        //
        ...
        try (URLClassLoader cl = new URLClassLoader(
            ...
            Method recoverMethod = parserClass.getMethod("recover", MetaDataLoader.class, String.class);
            ...
            Object result = recoverMethod.invoke(null, loader, dirty);
            ...
            assertFalse("hasLostRequired must be false — title recovered", hasLostRequired);
        }
    }
}
