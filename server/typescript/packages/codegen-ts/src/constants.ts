...
/**
 * What a generated CODE file's header says about editing it — stamped after
 * {@link GENERATED_HEADER} as `// <header> — <note>`.
 *
 * It must be TRUE on this port. It used to read `DO NOT EDIT.`, while `meta gen`
 * deliberately three-way-merges hand edits into the regenerated file (and, with no
 * snapshot body to merge against, refuses to overwrite an edited file rather than drop
 * the edit), and `meta verify --codegen` checks only each file's generated contribution.
 * The header forbade the workflow the tool is built around.
 *
 * Code files only. Generated Markdown docs keep their own marker: `meta verify --docs`
 * compares them whole.
 */
export const GENERATED_EDIT_NOTE =
  "hand edits are kept by a three-way merge on regeneration; meta verify --codegen checks only the generated parts.";

/**
 * The `<Entity>Names` artifact's filename suffix — `Subscriber.names.ts`.
 *
 * Spelled ONCE, because three places have to agree on it: the built-in generator that
 * emits the file, the reference template an adopter ejects, and `meta verify --codegen`,
 * which treats this artifact as fail-closed (a hand edit inside it IS drift, unlike every
 * other generated file). Three literals would be three chances for the gate to stop
 * finding the artifact it exists to protect, silently.
 */
export const NAMES_FILE_SUFFIX = ".names.ts";
