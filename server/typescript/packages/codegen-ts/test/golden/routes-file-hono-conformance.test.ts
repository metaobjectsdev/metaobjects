// Conformance check: routesFileHono() codegen output matches
// fixtures/conformance/<name>/expected/<Entity>.routes.hono.ts
//
// For every conformance fixture that ships an expected/<Entity>.routes.hono.ts
...
// Discover fixtures that have any expected/*.routes.hono.ts file.
const fixtures = readdirSync(CORPUS, { withFileTypes: true })
  ...
  .filter((name) => {
    ...
    return readdirSync(expectedDir).some((f) => f.endsWith(".routes.hono.ts"));
  });

describe("routesFileHono() conformance — expected/<Entity>.routes.hono.ts byte-match", () => {
  if (fixtures.length === 0) {
    it("no fixtures have expected/<Entity>.routes.hono.ts yet (placeholder)", () => {
      expect(true).toBe(true);
    });
  }

  for (const fixtureName of fixtures) {
    it(`${fixtureName} — Hono routes file matches byte-for-byte`, async () => {
      ...
      // Match each expected file by filename.
      const expectedFiles = readdirSync(expectedDir).filter((f) => f.endsWith(".routes.hono.ts"));
