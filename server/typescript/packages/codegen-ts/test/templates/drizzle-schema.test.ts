...
import { mapColumnType, PG_IDENTITY_CAPABLE_FNS } from "../../src/column-mapper.js";
...
describe("renderDrizzleSchema — Postgres", () => {
  test("emits pgTable with bigint identity PK for long id", () => {
    const root = makeRoot([makePost()]);
    const ctx = makeRenderContext({
      dialect: "postgres",
      loadedRoot: root,
  ...
  // An increment PK must keep the type the column-mapper derived and GAIN the
  // identity clause — it must not be given a second spelling of its own type.
  //
  // These assertions name no SQL type and no Drizzle function. They ask
  // `mapColumnType` what the column IS and require the emitted call to be that,
  // so the template cannot substitute a type behind the mapper's back. That is
  // exactly what it used to do: `serial`/`bigserial` here against the mapper's
  // `integer`/`bigint`, while migrate wrote `… GENERATED BY DEFAULT AS
  // IDENTITY` for the same field. `serial` is sugar for integer + an owned
  // sequence + a DEFAULT nextval(...) — a different column from an identity
  // column — so the generated schema misrepresented the live PK of every
  // integer-keyed table. The cross-package half (mapper vs migrate) is gated by
  // integration-tests/test/pk-identity-cross-door.test.ts; this is the half
  // that watches the emitter.
  for (const subType of [FIELD_SUBTYPE_INT, FIELD_SUBTYPE_LONG]) {
    test(`Postgres increment PK on field.${subType} emits the MAPPED column fn + identity clause`, () => {
      const entity = metaObject(OBJECT_SUBTYPE_ENTITY, "Keyed");
      attachRdbSource(entity, "keyed");
      const idField = metaField(subType, "id");
      entity.addChild(idField);
      const primary = meta(new TypeId(TYPE_IDENTITY, IDENTITY_SUBTYPE_PRIMARY), "primary");
      primary.setAttr("fields", ["id"]);
      primary.setAttr("generation", "increment");
      entity.addChild(primary);
      const root = makeRoot([entity]);
      const ctx = makeRenderContext({
        dialect: "postgres",
        loadedRoot: root,
        outDir: "/x",
      ...
      const out = renderDrizzleSchema(root.findObject("Keyed")!, ctx).toString();

      // What the mapper says this column is — the single source for the type.
      const expectedFn = mapColumnType(idField, "postgres").fnName;
      expect(PG_IDENTITY_CAPABLE_FNS.has(expectedFn)).toBe(true);

      // What the template actually emitted for that column.
      const emitted = /\bid:\s*([A-Za-z0-9_]+)\(/.exec(out)?.[1];
      expect(emitted).toBe(expectedFn);
      expect(out).toContain(".generatedByDefaultAsIdentity()");
    });
  }

  test("Postgres increment PK emits no serial/bigserial sugar in any form", () => {
    const root = makeRoot([makePost()]);
    const ctx = makeRenderContext({
      dialect: "postgres",
      loadedRoot: root,
    ...
    const out = renderDrizzleSchema(root.findObject("Post")!, ctx).toString();
    expect(out).not.toMatch(/\b(?:big|small)?serial\(/);
  });
