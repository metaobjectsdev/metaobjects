...
describe("renderZodValidators", () => {
  ...
  /** A Customer with one extra field carrying `@mutability: <mode>`. */
  function customerWithMutability(mode: string | undefined, fieldName: string) {
    const customer = metaObject(OBJECT_SUBTYPE_ENTITY, "Customer");
    customer.addChild(metaField(FIELD_SUBTYPE_LONG, "id"));
    customer.addChild(metaField(FIELD_SUBTYPE_STRING, "name"));
    const extra = metaField(FIELD_SUBTYPE_LONG, fieldName);
    if (mode !== undefined) extra.setAttr("mutability", mode);
    customer.addChild(extra);
    const primary = meta(new TypeId(TYPE_IDENTITY, IDENTITY_SUBTYPE_PRIMARY), "primary");
    primary.setAttr("fields", ["id"]);
    primary.setAttr("generation", "increment");
    customer.addChild(primary);
    return renderZodValidators(customer).toString();
  }

  /** The body of one named schema, so a field can be asserted present in ONE
   *  schema and absent from ANOTHER — which `expect(out).toContain` cannot do. */
  function schemaBody(out: string, name: string): string {
    // Split rather than regex: the emitted schema is one `export const <Name> = ...`
    // block, so the next `export const` (or EOF) is its end.
  ...
  test("FR-037 R1: @mutability readOnly is excluded from both InsertSchema and UpdateSchema", () => {
    const out = customerWithMutability("readOnly", "lifetimeValue");
    expect(out).toContain("export const CustomerInsertSchema");
    expect(out).toContain("export const CustomerUpdateSchema");
    // Nobody writes it — it appears in NEITHER input schema.
    expect(out).not.toContain("lifetimeValue");
  });

  test("FR-037 R1: @mutability writeOnce is IN the Insert schema and OUT of the Update schema", () => {
    // The mode's entire point, and the case the boolean vocabulary could not
    // express: settable exactly once. Asserting per-schema rather than over the
    // whole file, because "present here, absent there" is the actual contract.
    const out = customerWithMutability("writeOnce", "issuedCurrency");
    expect(schemaBody(out, "CustomerInsertSchema")).toContain("issuedCurrency");
    expect(schemaBody(out, "CustomerUpdateSchema")).not.toContain("issuedCurrency");
  });

  test("FR-037 R1: writeOnce is not reachable through the FR-035 present-null clear either", () => {
    // Clearing a column IS a write, so the PATCH tristate must not offer a
    // `.nullable()` escape hatch on a writeOnce field — it is absent outright.
    const out = customerWithMutability("writeOnce", "issuedCurrency");
    const update = schemaBody(out, "CustomerUpdateSchema");
    expect(update).not.toContain("issuedCurrency");
    // The sibling readWrite field still gets its present-null clear, proving the
    // assertion above is about the MODE and not about the schema being empty.
    expect(update).toContain("name");
  });

  test("FR-037 R1: no @mutability (⇒ readWrite) is in BOTH schemas — the no-churn default", () => {
    const out = customerWithMutability(undefined, "lifetimeValue");
    expect(schemaBody(out, "CustomerInsertSchema")).toContain("lifetimeValue");
    expect(schemaBody(out, "CustomerUpdateSchema")).toContain("lifetimeValue");
  });

  test("FR-037 R1: an EXPLICIT readWrite emits byte-identically to no @mutability at all", () => {
    // Output equivalence pin: the attr is inert at its default, so adding it to a
    // model can never move generated bytes.
    expect(customerWithMutability("readWrite", "lifetimeValue"))
      .toBe(customerWithMutability(undefined, "lifetimeValue"));
  });
