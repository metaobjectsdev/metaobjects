...
describe("init scaffolds new layout (Task 11 contract)", () => {
  test("creates metaobjects/, .metaobjects/, metaobjects.config.ts — not legacy paths", async () => {
    ...
    try {
      ...
      expect(existsSync(join(tmp, ".metaobjects"))).toBe(true);
      expect(existsSync(join(tmp, ".metaobjects", "config.json"))).toBe(true);
      expect(existsSync(join(tmp, ".metaobjects", ".gen-state"))).toBe(true);
