...
describe("parseVerifyArgs", () => {
  test("defaults: prompts/db/dialect undefined, allow empty, skipSchema false, no explicit subverb", () => {
    expect(parseVerifyArgs([])).toEqual({
      ...
      templates: false, codegen: false, forbidHandEdits: false, docs: false, deps: false, anyExplicit: false, noAntipatterns: false, noRequirementLint: false, noOverlayLint: false, lax: false,
  ...
  test("--db / --dialect / --skip-schema are captured", () => {
    expect(parseVerifyArgs(["--db", "file:x.db", "--dialect", "sqlite", "--skip-schema"])).toEqual({
      ...
      templates: false, codegen: false, forbidHandEdits: false, docs: false, deps: false, anyExplicit: true, noAntipatterns: false, noRequirementLint: false, noOverlayLint: false, lax: false,
      replay: false, replaySnapshot: false,
      d1: undefined, remote: false, limit: DEFAULT_ADVISORY_LIMIT,
    });
  });
  ...
  test("--docs selects the docs gate and nothing else", () => {
    const f = parseVerifyArgs(["--docs"]);
    expect(f.docs).toBe(true);
    expect(f.codegen).toBe(false);
    expect(f.templates).toBe(false);
    // It is a subverb, so passing it suppresses the bare-verify template default.
    expect(f.anyExplicit).toBe(true);
    expect(parseVerifyArgs([]).docs).toBe(false);
  });
  ...
  // Task 15 — --deps is its own subverb, same shape as --codegen/--docs: it
  // counts toward anyExplicit (so a lone `verify --deps` does not ALSO run the
  // bare-verify template default) but is never itself part of that default.
  test("--deps sets the explicit-subverb flags and nothing else", () => {
    const f = parseVerifyArgs(["--deps"]);
    expect(f.deps).toBe(true);
    expect(f.templates).toBe(false);
    expect(f.codegen).toBe(false);
    expect(f.docs).toBe(false);
    expect(f.anyExplicit).toBe(true);
    expect(parseVerifyArgs([]).deps).toBe(false);
  });
  ...
  test("--forbid-hand-edits rides on --codegen, and is refused without it", () => {
    expect(parseVerifyArgs(["--codegen"]).forbidHandEdits).toBe(false);
    expect(parseVerifyArgs(["--codegen", "--forbid-hand-edits"]).forbidHandEdits).toBe(true);
    expect(() => parseVerifyArgs(["--forbid-hand-edits"])).toThrow(/only applies to the codegen gate/);
  });
