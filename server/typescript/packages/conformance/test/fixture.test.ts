...
test("discovers a scenario with a script and reports its providers", async () => {
	...
	expect(fixtures[0]!.providers).toEqual([
...
test("a contract-only fixture (expected-paths.json, no strict files) is detected + classified", async () => {
	const root = await corpus({
		"api-docs-cross-port/input/meta.json": "{}",
		"api-docs-cross-port/expected-paths.json": '{"units":[]}',
	});
	const fixtures = await discoverFixtures(root);
	const fix = fixtures[0]!;
	expect(fix.hasContractManifest).toBe(true);
	expect(fix.hasExpected).toBe(false);
	expect(isContractOnlyFixture(fix)).toBe(true);
	// It is NOT a docs-only fixture (no expected/*.md).
	expect(isDocsOnlyFixture(fix)).toBe(false);
});

test("a strict fixture that ALSO carries a contract manifest is NOT contract-only", async () => {
	const root = await corpus({
		"mixed/input/meta.json": "{}",
		"mixed/expected.json": "{}",
		"mixed/expected-paths.json": '{"units":[]}',
	});
	const fixtures = await discoverFixtures(root);
	const fix = fixtures[0]!;
	expect(fix.hasContractManifest).toBe(true);
	// Strict expectation present ⇒ the strict runner must still run it.
	expect(isContractOnlyFixture(fix)).toBe(false);
});
...
test("a malformed providers.json (non-array) causes discoverFixtures to reject", async () => {
	...
	await expect(discoverFixtures(root)).rejects.toThrow(
		/providers\.json must be a JSON array of strings/,
	);
});
