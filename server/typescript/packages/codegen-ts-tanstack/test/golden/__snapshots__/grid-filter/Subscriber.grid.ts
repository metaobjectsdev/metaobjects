// @generated by @metaobjectsdev/codegen-ts-tanstack — hand edits are kept by a three-way merge on regeneration; meta verify --codegen checks only the generated parts.
// Source metadata: Subscriber (Subscriber)
import type { Subscriber as SubscriberRow } from "./Subscriber";
import { Subscriber } from "./Subscriber.meta";
import { subscriberActiveFilter } from "./Subscriber.columns";
import { buildFilterQs } from "@metaobjectsdev/runtime-web";
import { useEntityPathFetcher } from "@metaobjectsdev/tanstack";
import { useQuery } from "@tanstack/react-query";
import type {
  ColumnFiltersState,
  PaginationState,
  SortingState,
} from "@tanstack/react-table";
import { useMemo, useState } from "react";

export function useSubscriberActiveGrid() {
  const [sorting, setSorting] = useState<SortingState>([]);
  const [pagination, setPagination] = useState<PaginationState>({
    pageIndex: 0,
    pageSize: 25,
  });
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [search, setSearch] = useState<string>("");

  const fetcher = useEntityPathFetcher();

  const qs = useMemo(() => {
    const filterObj: Record<string, unknown> = { ...subscriberActiveFilter };
    for (const f of columnFilters) {
      filterObj[f.id] = f.value as unknown;
    }
    const sortEntry = sorting[0];
    const sort = sortEntry
      ? `${sortEntry.id}:${sortEntry.desc ? "desc" : "asc"}`
      : undefined;
    return buildFilterQs({
      ...filterObj,
      ...(sort !== undefined ? { sort } : {}),
      limit: pagination.pageSize,
      offset: pagination.pageIndex * pagination.pageSize,
      ...(search !== "" ? { search } : {}),
      withCount: 1,
    });
  }, [sorting, pagination, columnFilters, search]);

  const query = useQuery<{ rows: SubscriberRow[]; total: number }>({
    queryKey: ["subscriber", "grid", "active", qs],
    queryFn: () =>
      fetcher<{ rows: SubscriberRow[]; total: number }>(
        `${Subscriber.$path}?${qs}`,
      ),
  });

  return {
    data: query.data?.rows ?? [],
    rowCount: query.data?.total ?? 0,
    state: { sorting, pagination, columnFilters },
    onSortingChange: setSorting,
    onPaginationChange: setPagination,
    onColumnFiltersChange: setColumnFilters,
    search,
    onSearchChange: setSearch,
    isLoading: query.isLoading,
    error: query.error,
  };
}
