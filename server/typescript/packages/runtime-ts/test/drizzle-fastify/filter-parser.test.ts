...
describe("parseFilterParams — happy path", () => {
  ...
  // Cross-port contract (ADR-0047): the `like` OP is case-SENSITIVE SQL LIKE
  ...
  test("like on postgres uses case-sensitive like — never ilike (ADR-0047)", () => {
    const r = parseFilterParams({
  ...
  test("like on sqlite lowers to GLOB with a translated pattern (SQLite LIKE folds ASCII case) — ADR-0047", () => {
    const r = parseFilterParams({
