...
describe("SQLite recreate-and-copy — data preservation", () => {
  test("change-column-default preserves rows", async () => {
    ...
    const metadata1 = (await new MetaDataLoader().load([new InMemoryStringSource(JSON.stringify(json1))])).root;
    ...
    const metadata2 = (await new MetaDataLoader().load([new InMemoryStringSource(JSON.stringify(json2))])).root;
