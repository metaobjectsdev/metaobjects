...
// "path under library/ minus .yaml" and every reader appends `.yaml` back. Folding a
// `.json` into that would mean a special case at every call site in four ports; one more
// constant means none.
const manifests = readdirSync(libDir, { withFileTypes: true })
  .filter((e) => e.isDirectory())
  .map((e) => ({ name: e.name, path: join(libDir, e.name, "library.json") }))
  .filter((m) => existsSync(m.path))
  .map((m) => ({ name: m.name, content: readFileSync(m.path, "utf-8") }))
  .sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));

// A library directory with no manifest is a build error, not a silently-skipped one: the
// manifest is what declares the library's LAYERS, and without it `libraries: ["iam/db"]`
// resolves to nothing and the adopter gets an empty tree with no explanation.
const withoutManifest = readdirSync(libDir, { withFileTypes: true })
  .filter((e) => e.isDirectory() && !existsSync(join(libDir, e.name, "library.json")))
  .map((e) => e.name);
if (withoutManifest.length > 0) {
  console.error(`error: library director${withoutManifest.length === 1 ? "y" : "ies"} with no library.json: ${withoutManifest.join(", ")}`);
  process.exit(1);
}
...
${manifests.map((m) => `  ${JSON.stringify(m.name)}: ${JSON.stringify(m.content)},`).join("\n")}
};
`;
...
public final class EmbeddedLibrary {
    ...
    /** Library name to the exact text of its {@code library.json} manifest. */
    public static final Map<String, String> MANIFESTS;
...
${manifests.map((x) => `        n.put(${JSON.stringify(x.name)}, ${JSON.stringify(x.content)});`).join("\n")}
        MANIFESTS = Collections.unmodifiableMap(n);
    }
}
`;
...
${csharpEntries}
    ...
    /// <summary>Library name to the exact text of its <c>library.json</c> manifest.</summary>
    public static readonly IReadOnlyDictionary<string, string> Manifests =
        new Dictionary<string, string>
    {
${manifests.map((x) => `        [${JSON.stringify(x.name)}] = ${JSON.stringify(x.content)},`).join("\n")}
    };
}
`;
...
console.log(
  `wrote ${entries.length} embedded library file(s) + ${manifests.length} manifest(s) to ` +
    `${relative(repoRoot, outFile)}, ${relative(repoRoot, javaOutFile)} and ` +
    `${relative(repoRoot, csharpOutFile)}`,
);
