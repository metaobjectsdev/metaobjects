...
| `names` | `<entity_snake>_names.py` — module-level `Final` constants for the object's physical database names (`<ENTITY>_KIND`/`_NAME`/`_SCHEMA`/`_READ_ONLY`, a `_<FIELD>_FIELD`/`_<FIELD>_COLUMN` pair each, and a complete `_COLUMNS_BY_FIELD`). Emitted for every object with a declared or inherited primary source, PLUS a fragment for any abstract base such an object extends (columns only, no `_NAME`). Python has no static inheritance, so a module whose object extends another **imports and re-exports** the parent's constants (`AUTHOR_CREATED_AT_COLUMN: Final[str] = BASEENTITY_CREATED_AT_COLUMN`) instead of restating the literal. **This port generates no SQL**, so nothing generated consumes these — they exist for the repository `Protocol` implementation you write. |
