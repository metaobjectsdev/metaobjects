...
publish candidates), Maven Central `8.0.5`, PyPI `1.0.5`, NuGet `1.0.5` — every port has changed
product code._
...
- **A `@cardinality: one` relationship declared on a TPH subtype (or an abstract mid level)
  ...
  tier**: for a CONCRETE discriminator base with a writable source, the subtype's unit loses
  its `<Sub>Relations` symbol — it documented an export that never existed, since a subtype
  emits a value-object module with no relations block — and the base's relations symbol
  gains the folded subtype-declared navigations, which are real. For an ABSTRACT
  discriminator base, or one with no writable source, no unit in the hierarchy carries a
  relations symbol at all, because none was ever real there: both shapes route to the
  value-object emission, so no module in the hierarchy renders a block — and a plain
  sourceless entity carrying a belongs-to loses the same long-standing phantom symbol. An
  abstract level with no concrete descendant beneath it likewise files nothing —
