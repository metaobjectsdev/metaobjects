 ...
 * build at all**. `@metaobjectsdev/tanstack` sits directly downstream of `runtime-web`
 * and is what a generated `<Entity>.hooks.ts` / `<Entity>.grid.ts` imports — it
 * inherited the break and was ungated. This is that gate. The full post-mortem lives in
...
describe("#287 — browser bundleability", () => {
  test("the BUILT tanstack entry bundles for a browser target", async () => {
