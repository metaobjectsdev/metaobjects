 ...
 * 0.21.3 `runtime-web` had one, so no client consuming the generated code could build.
 * `@metaobjectsdev/angular` depends on `runtime-web`, so it carries the same exposure —
 * and it was the least-gated of the browser packages: its only other test cannot run at
 * all under Bun (`Standard Angular field decorators are not supported in JIT mode`), which
 * is why the CI lane runs THIS file by name rather than the package's suite. The full
 * post-mortem lives in `runtime-web/test/browser-bundleable.test.ts`; the short version:
 *
 * Bun's TEST runner resolves the `"bun"` export condition to TypeScript SOURCE and never
 * bundles, so a passing test says nothing about the `dist` path a published consumer
 * resolves. The one thing that reproduces the bug is a real browser-target bundle over the
 * BUILT output — hence `dist/`, deliberately, not `src/`.
 *
 * Verified by re-introducing the regression (a root-barrel value re-export in
 * runtime-web's dist): the sibling gates fail with the original message verbatim.
 */
...
describe("#287 — browser bundleability", () => {
  test("the BUILT angular entry bundles for a browser target", async () => {
