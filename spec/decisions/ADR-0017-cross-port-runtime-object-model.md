...
**Applies to:** All five ports (TypeScript reference + Java / Python / C# / Kotlin); the runtime layer that instantiates and manipulates metadata-described objects (recover, serializers, runtime metadata access).
...
- **Consistent runtime metadata manipulation across all ports.** recover + serializers build directly on `newInstance` + get/set, so they port off the JVM without per-port special-casing.
...
- **Phase B builds on this**: metadata-driven `recover` and a generalized `@default` materialize and populate instances through this model.
...
- **Leave the model JVM-only and special-case recover/serializers per port.** Rejected — it duplicates the materialize-and-populate logic in every port and diverges behavior; a shared object model is the smaller, conformance-pinnable surface.
