> **ARCHIVED — superseded by [FR-037](../../../docs/superpowers/specs/2026-08-10-fr-037-projection-expressiveness-and-write-once-design.md).**
>
> This is the raw findings doc from the requirements-modeling spike. It is kept for its
> provenance — the replication counts (e.g. "8 of 8 runs") and the verbatim agent quotes are
> the evidence FR-037's sequencing argument rests on — but it is **no longer the live
> document**. Do not plan from it.
>
> What became requirements in FR-037:
> **G4** -> R1 `@writeOnce` · **G1** -> R3 `origin.rank` · **G3** -> R4 (an acceptance case of
> R3, not a feature) · and the escape-valve composition problem noted under G1, promoted out
> of that footnote to **R2** (a per-field SQL escape) because it bounds the blast radius of
> every future irreducible column independently of whether G1 ships.
>
> What stayed parked, with reasons recorded in FR-037: **G2** (re-scoped — `attr.filter`
> makes the syntax free, not the semantics, so it is sized like R3, not like R1), **G5**
> (overlaps FR-024 `operation.command`), **G6** (blocked on FR-024; `api.*` has zero
> registered subtypes).
>
> **One error in the text below**, corrected in FR-037: G3 lists `first` as a member of the
> `origin.aggregate` `@agg` set. It is not — `first` is an origin *subtype*. The registered
> `@agg` members are `count, sum, avg, min, max, any, all, collect`. Every other premise in
> this doc was verified against `fixtures/registry-conformance/expected-registry.json` and
> `validate-source-escapes.ts` and held.
