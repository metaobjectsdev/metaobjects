...
**Overlay application is a deferred pass** (ADR-0055). Load order fixes the *contributor* order above, but it does not decide whether an `overlay: true` declaration finds its target: every port parses every source first, applying only plain declarations (**G1**), then applies every queued overlay in source order, declaration order within a source (**G2**). So an overlay resolves against a base declared in any file — including a later-sorted one, or the same file above it — and `ERR_OVERLAY_NO_TARGET` means the target is *absent*, never *not parsed yet*. The `overlay-*` fixtures pin both guarantees; `overlay-applies-after-all-plain-declarations` is the one that pins G1 specifically.
