...
The substrate is local-first: typed metadata lives in your repo at `metaobjects/`, generated code is idiomatic per-language output with **no proprietary runtime**: entity and model code imports nothing from MetaObjects, and the REST, prompt, client and runtime tiers import ordinary Apache-2.0 packages. If `@metaobjectsdev/*` packages disappeared tomorrow, your generated code keeps working.
