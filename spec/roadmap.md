...
| FR-023 | Metadata dependencies (cross-repo sharing: generated shared-model artifact, `meta deps sync`, committed snapshot + lock, default exclusion of imported metadata, consumer extend/overlay across the boundary) | 🟢 Phase 1a (TS + Python) **shipped** per DESIGN §11 — the usage-aware breaking-change classifier and the `npm`/`python` transports are deferred (§11.4); Java/Kotlin/C# consumers are Phase 2 | 1.1 | [#9](https://github.com/metaobjectsdev/metaobjects/issues/9) |
...
| FR-041 | Public A/B drift benchmark — coding agents with vs without MetaObjects, pre-registered, friction-first | 📋 **design settled 2026-09-12, unbuilt** — the "proving the value" work; it is what licenses the claims FR-042 §4 withholds. Revised after an adversarial two-reviewer design review (spec §14): the scored task set is now held out from the friction pass (the draft tuned Arm B on the tasks it would later be scored on, with no symmetric loop for the control), Arm A is derived from Arm B's generated output so the seeds differ only in the model and the gate, `n` is calibrated from a pilot instead of asserted (one identical config measured 25/51/28 turns at temperature 0 — 41% CV), H2 is time-to-**correct** rather than time-to-done, the primary is analysed intention-to-treat so an arm cannot win by not finishing, and escaped defects are reported split by whether `meta verify` already covers the invariant class (it covers four of five, so H1 is partly definitional for those). First deliverable is the Phase 0b friction log. Design: `docs/superpowers/specs/2026-09-11-fr-041-drift-ab-benchmark-design.md` | 1.x | — |
| FR-042 | First-touch positioning — one typed model, two verbs (README, llms, sites) | 🚧 in progress — pitch **locked** 2026-09-12 (two verbs: Generate + Verify; requirements fold into Verify; H1 model-first). Implementing across the four first-touch surfaces. Design: `docs/superpowers/specs/2026-09-11-fr-042-first-touch-positioning-design.md` | — | — |
| FR-043 | Feature & non-functional packages — shippable requirements + model metadata (+ generator selection) an agent pulls in and adapts | 📋 **design sketch 2026-09-13, deferred by ruling** — not greenfield: `library/ai/llm-call.yaml` already ships one package (the LLM-call audit model, opt-in via the loader's `libraries: ["ai"]`, embedded per port under an `embedded-library drift` gate, with `trace-helper` codegen and OMDB runtime beside it). The FR generalises it: requirements as a component, discovery through the codegen catalog's `kind`, declared package→generator coupling in place of `trace-helper`'s hard-coded `LlmCallBase`, copy-and-own via eject, and third-party authoring riding FR-023's deferred transports. Deferred until FR-041 can measure whether declared design actually helps. Design: `docs/superpowers/specs/2026-09-13-fr-043-feature-and-nfr-packages-design.md` | later / adjacent project | — |
...
- **FR-023 — Metadata dependencies: cross-repo distribution + reuse.** **Phase 1a
  shipped** (TS + Python, per DESIGN §11's "B+" ruling — thin dependencies plus
  default exclusion of imported metadata): a publisher's `sharedModelFile()`
  ...
  declares a brand-new node into a package it does not own. Overlay/`extends`
  across the boundary is expected and documented. **Superseded from the v2 design**
  (§11.3): per-dependency `mode: reference | own`, the usage-aware breaking-change
  classifier + `--accept-breaking`, and node-level (rather than package-level)
  ownership. **Still deferred** (§11.4): the `npm`/`python`/`maven`/`nuget`
  transports, a local co-development override, `packageBindings` (cross-boundary
  codegen imports), a runtime `ObjectManager` scope predicate, and Java/Kotlin/C# as
  consumers or publishers (Phase 2). Design: `docs/superpowers/specs/2026-09-11-fr-023-metadata-dependencies-design.md`
  (supersedes the 2026-06-11 sketch). Docs: `docs/features/metadata-dependencies.md`.
  Corpus: `fixtures/dependency-conformance/` (23 cases, TS + Python).
...
`docs/RELEASING.md:167` states the mechanism — while the project is 0.x, a **MINOR is how a
breaking change ships**, because `^0.20.x` does not resolve `0.21.0` and so the break is adopted
...
- **FR-023 — metadata dependencies, remaining phases** (Phase 1a — TS + Python —
  ...
  `npm`/`python`/`maven`/`nuget` transports — remain).
