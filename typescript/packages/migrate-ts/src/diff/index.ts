...
export async function diff(
  ...
  for (const [name, table] of expectedTables) {
    if (!actualTables.has(name)) {
      changes.push({ kind: "create-table", table, status: ALLOWED });
      for (const index of table.indexes) {
        changes.push({ kind: "add-index", table: name, index, status: ALLOWED });
      }
      for (const fk of table.foreignKeys) {
        changes.push({ kind: "add-fk", table: name, fk, status: ALLOWED });
      }
    }
  }
  ...
  for (const [name, expectedTable] of expectedTables) {
    ...
    diffTableColumns(name, expectedTable, actualTable, changes);
    diffTableIndexes(name, expectedTable, actualTable, changes);
    diffTableForeignKeys(name, expectedTable, actualTable, changes);
  }
  ...
  for (const [name, ac] of actualCols) {
    if (!expectedCols.has(name)) {
      const dropChange: Change & { _sqlType?: SqlType; _nullable?: boolean } = {
        kind: "drop-column", table, column: name, status: ALLOWED,
      };
...
function diffTableIndexes(
  ...
  for (const [name, ix] of expectedIdx) {
    ...
    if (!a) {
      changes.push({ kind: "add-index", table, index: ix, status: ALLOWED });
    } else if (!indexEquals(ix, a)) {
      // Index shape changed: drop + add (atomic from caller's perspective).
      changes.push({ kind: "drop-index", table, index: name, status: ALLOWED });
      changes.push({ kind: "add-index", table, index: ix, status: ALLOWED });
    }
  }
...
function diffTableForeignKeys(
  ...
  for (const [name, fk] of expectedFk) {
    ...
    if (!a) {
      changes.push({ kind: "add-fk", table, fk, status: ALLOWED });
    } else if (!fkEquals(fk, a)) {
      changes.push({ kind: "drop-fk", table, fk: name, status: ALLOWED });
      changes.push({ kind: "add-fk", table, fk, status: ALLOWED });
    }
  }
