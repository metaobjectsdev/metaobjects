...
describe("emit (postgres) — schema namespacing end-to-end", () => {
  describe("emits CREATE TABLE schema.table for a non-default schema", () => {
          ...
          "children": [
            {
              "object.entity": {
                ...
                "children": [
                  { "field.long": { "name": "id" } },
                  { "source.dbView": { "name": "src", "@name": "cases_v1", "@schema": "p3_api" } },
    ...
    test("schema-qualified and unqualified CREATE TABLE statements", async () => {
      ...
      // resolveTableName falls back to pluralize(toSnakeCase("CaseV1")) = "case_v1s"
      // because resolveTableName only looks at source.dbTable, not source.dbView.
      expect(sql.up).toMatch(/CREATE TABLE "p3_api"\."case_v1s"/);
