...
describe("diff — status enforcement", () => {
  test("drop-column blocked by default", async () => {
    const e = snap([{ name: "u", columns: [col("id", { kind: "integer", bits: 64 })], indexes: [], foreignKeys: [], primaryKey: ["id"] }]);
    const a = snap([{ name: "u", columns: [col("id", { kind: "integer", bits: 64 }), col("legacy", { kind: "text" })], indexes: [], foreignKeys: [], primaryKey: ["id"] }]);
  ...
  test("drop-table blocked by default; allowed with allow.dropTable", async () => {
    const e = snap([]);
    const a = snap([{ name: "legacy", columns: [], indexes: [], foreignKeys: [], primaryKey: [] }]);
  ...
  test("change-column-type widening (int32→int64) allowed without flag", async () => {
    const e = snap([{ name: "t", columns: [col("n", { kind: "integer", bits: 64 })], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const a = snap([{ name: "t", columns: [col("n", { kind: "integer", bits: 32 })], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const r = await diff(e, a);
  ...
  test("change-column-type narrowing (int64→int32) blocked", async () => {
    const e = snap([{ name: "t", columns: [col("n", { kind: "integer", bits: 32 })], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const a = snap([{ name: "t", columns: [col("n", { kind: "integer", bits: 64 })], indexes: [], foreignKeys: [], primaryKey: [] }]);
  ...
  test("change-column-nullable: notnull→nullable allowed", async () => {
    const e = snap([{ name: "t", columns: [col("c", { kind: "text" }, true)], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const a = snap([{ name: "t", columns: [col("c", { kind: "text" }, false)], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const r = await diff(e, a);
  ...
  test("change-column-nullable: nullable→notnull blocked without flag", async () => {
    const e = snap([{ name: "t", columns: [col("c", { kind: "text" }, false)], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const a = snap([{ name: "t", columns: [col("c", { kind: "text" }, true)], indexes: [], foreignKeys: [], primaryKey: [] }]);
  ...
  test("drop-index / drop-fk blocked by default", async () => {
    const e = snap([{ name: "t", columns: [], indexes: [], foreignKeys: [], primaryKey: [] }]);
    const a = snap([{
      name: "t", columns: [],
