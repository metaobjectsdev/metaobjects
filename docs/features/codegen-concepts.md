...
There is no default set to start from, by design (ADR-0034 Amendment 2): `meta init`
scaffolds `codegen/generators/` empty with `generators: []`. `meta gen --list --probe` is
the catalog — every generator, with how many files it would emit for your model — and
`meta eject <name>...` copies the ones you choose from the reference templates into
`codegen/generators/`, printing the import and entry to wire, so `meta gen` runs from
generators you own from the first run. `eject` never clobbers a file you already own. The
old `@metaobjectsdev/codegen-ts/generators` import of `entityFile` / `queriesFile` /
`routesFile` / `barrel` was **removed at 1.0** — own the local copy.
...
- **C# and Python** decide the same way — from the committed `.hashes.json` — and refuse
  rather than merge, since neither keeps a snapshot body to merge against. Their headers
  (`<auto-generated/>` and `@generated by metaobjects`) are informational too **whenever a
  gen-state directory is in play, which is every CLI run**. One caller can reach the
  exception: an *embedder* driving the C# `CodegenRunner` or Python's `decide_and_write`
  directly and supplying no gen-state directory. With no state to reason from there is no
  hash to compare, so both fall back to the JVM's rule — overwrite a file carrying the
  marker, refuse one without it — and in that mode the header IS load-bearing, in both
  directions. Neither `dotnet meta gen` nor `metaobjects gen` can put you there: both
  always supply the directory. If you embed either runner, supply one too.
