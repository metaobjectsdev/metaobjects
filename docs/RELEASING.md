...
Four packages, version-locked at the C# port version (currently `1.0.3`):
...
currently `1.0.3`), as an **sdist + a universal `py3-none-any` wheel** (pure Python).
...
1.0 cut (currently `8.0.3`) — in the parent + module poms. Signed with the maintainer's GPG key.
