...
Four packages, version-locked at the C# port version (currently `0.22.0`):
...
currently `0.22.0`), as an **sdist + a universal `py3-none-any` wheel** (pure Python).
...
versioned on the `7.x` line (currently `7.22.0`) in the parent + module poms. Signed with the
