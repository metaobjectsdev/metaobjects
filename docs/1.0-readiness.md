...
- ✅ **G3. A quiet period** — at least one coordinated release after the last breaking
  ...
  **RULED (2026-08-26, maintainer): `0.24.2` does NOT reset the clock, and IS the quiet
  release G3 asks for.** FR-039 moves `metamodelVersion` `0.12` → `0.13`, and that is the axis
  G3 measures — but the move is **ADDITIVE**. `planned | live | partial` keep their meanings
  and their gates, `@supersededBy` returns on a status that did not previously exist, and
  **no document that loads on `0.24.1` stops loading**. Zero adopters must edit anything.

  G3's stated purpose is *"to prove the rate has actually dropped"*, and the rate it measures
  is BREAKING churn — the thing that costs an adopter a migration. A release forcing no edits
  is evidence the rate dropped, not evidence against it. That is the distinction the `0.24.1`
  discussion above could not settle cleanly, because `0.24.1` **did** force edits (two
  previously-loading forms stopped loading) whatever one calls them; `0.24.2` forces none, so
  the two cases do not need the same answer.

  **The asymmetric-cost argument above still applies and is answered rather than waived:** the
  risk of ruling wrongly here is cutting 1.0 off a window that was actually churning. It was
  not. `0.24.1` is treated as NOT satisfying G3 (per the recommendation), and `0.24.2` — one
  coordinated release later, additive, adopter-edit-free — is the one that does. So G3 is
  **satisfied by `0.24.2`**, and G1 is unblocked on this gate.
  ...
  **This cost was adjudicated, not discovered.** The batch was ruled in knowing it would
  ...
  than an accident.
  **RESET (2026-09-06): `0.25.0` restarts the clock, so G3 is no longer satisfied.** It
  moves `metamodelVersion` `0.13` → `0.14`, and unlike `0.24.2` the move is **NOT additive**:
  an authored `<type>.base` node now fails the load with `ERR_ABSTRACT_SUBTYPE_AUTHORED` in
  all five ports, and TypeScript, C# and Python **loaded every one of those before**. So a
  document that loads on `0.24.x` stops loading, which is exactly the test the `0.24.2`
  ruling used to distinguish a resetting release from a quiet one. Five further breaking
  changes ship beside it (the API base URL leaves the entity descriptor, a TPH subtype's `$path`,
  `<Entity>Names`' shape, the removed `@metaobjectsdev/sdk/agent-docs` subpath, Python's
  `GenConfig.column_naming`), and each forces adopter edits.

  Recorded here rather than left to be reconstructed, for the reason the paragraph above
  gives: the reset should read as a decision, not as something discovered later. **G3 now
  needs one more coordinated release after `0.25.0` with no breaking metamodel change.**

  The judgement that `0.25.0` was worth it is not re-litigated here — it spent the pre-1.0
  breaking slot on one theme (a name resolved twice by two different mechanisms), and
  batching six migrations into one window is the same trade `0.24.0` made. What this bullet
  records is only the consequence: the clock restarted on 2026-09-06.
