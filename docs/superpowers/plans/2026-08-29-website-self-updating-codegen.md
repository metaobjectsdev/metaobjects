...
- [x] **Step 5: Push the site repo, then verify live**
...
> **DONE 2026-08-30, after the `0.24.5` release unblocked it.** Order held as resequenced:
> release → site push → deploy. Verified before pushing by simulating the deploy exactly —
> clone at the tag the deploy's own filter resolves (`v0.24.5`), run `node
> scripts/site-inject-ci.mjs` against a copy of `www/` — which is worth keeping as the
> pre-push check, because it exercises the tag rather than the working tree.
>
> Live on metaobjects.dev: **15 placeholders across 3 pages, all filled, none empty**;
> **zero `<script>` tags** on every page; 8 `<details class="example-details">` expanders;
> 1561 highlight spans on the index; and the injected content carries `@generated by
> @metaobjectsdev/codegen-ts — DO NOT EDIT.`, so it is the real corpus, not a fixture.
>
> **The bootstrap escape self-extinguished, as designed and as required.**
> `site-bijection.test.ts` now takes the REAL path — the live site references 15
> placeholders, so the zero-placeholder branch cannot fire — and it passes with no warning
> printed. Confirmed independently by counting `data-snippet` on the live raw URLs rather
> than trusting the green.
>
> **One defect this exposed, which Task 14 must fix: the release tag is cut in the MIDDLE
> of a coordinated release, not at the end.** `scripts/release.mjs` Phase 8 tags right after
> the npm publish, and the PyPI/NuGet/Maven bump is a SEPARATE later commit — so
> `v0.24.5`'s payload carries `{npm: 0.24.5, pypi: 0.24.4, nuget: 0.24.4, maven: 7.24.4}`.
> Harmless for snippets, which is why it did not surface here. **Not harmless for Task 14**,
> whose whole subject is publishing those coordinates from the tag-pinned payload: the site
> would state three registries one release stale, every release, forever. Fix the ORDER
> (tag after the coordinated cut completes), not the payload.
...
> **DONE 2026-08-30 — all six steps. Step 5 was BLOCKED when this was written, on a deadlock
> the plan did not anticipate; it cleared with the `0.24.5` release and the live verification is
> recorded under step 5 above. The account below is kept because the deadlock is the durable
> lesson, not the block.** Monorepo `scripts/site-inject-ci.mjs`, `scripts/site-bijection.test.ts`,
> `scripts/site/inject.mjs`, `scripts/release.mjs`; site repo `deploy.yml` (pushed 2026-08-30
> alongside Task 12, at site `8faa597`).
>
