...
- **REGISTRATION is coordinated cross-port; LOGIC is TS-first (verified 2026-07-15).** `expected-registry.json` is a **single shared manifest with NO per-entry `ports`** — so the moment a new subtype/attr/`@agg` value is registered in ANY port (spec copy + provider class), that port's emitted manifest diverges from the shared file and its registry-conformance goes RED until the file is updated; and updating the shared file turns the OTHER four ports RED until they register too. **Consequence:** the vocab *registration* step (canonical spec + all 5 spec copies + all 5 provider registrations + `expected-registry.json` in one commit) must land ALL FIVE PORTS AT ONCE — it cannot be TS-first. What IS TS-first: the standalone *logic* (the `attr.expression` grammar functions, validation rules, view synthesis) — build + unit-test it with zero manifest impact (do NOT add the subtype to `ATTR_SUBTYPES` / call `registerAttrClass` / touch the manifest until the coordinated registration). **Task 0.1's grammar core (`meta-attr-expression.ts` functions) already shipped this way (`45868323`).** Restructure Phase 0/1: build all TS logic first (grammar, validation helpers, synthesis) behind the unregistered vocab; then do ONE coordinated cross-port registration (fork-per-port for each port's provider + spec copy) that flips registry-conformance green everywhere at once; then wire each port's validation/typing to the now-registered vocab.
