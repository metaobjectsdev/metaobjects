...
- [ ] **Step 3 — Relationship join vocab + drop `referencedBy`.** Register `joinEntity`/`joinFields`; grep `codegen-spring`/`codegen-kotlin`/`omdb` for `referencedBy` usage and migrate consumers to the join vocab (or a derived getter) before removing the registration. Keep persistence + api-contract conformance green.
