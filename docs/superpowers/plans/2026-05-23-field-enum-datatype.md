...
- **Java and Python** get **metamodel recognition + `@values` validation** (subtype + `@values` so the loader accepts and round-trips, PLUS the loader-level member-validation rules below). Their codegen tiers aren't built yet; the *vocabulary contract* + *validation contract* are what must land now.
- **Validation is a cross-language contract** (revised per review): the `@values` rules — must be a **non-empty** array, every member matches `ENUM_MEMBER_PATTERN` (`^[A-Za-z_][A-Za-z0-9_]*$`), and **no duplicate** members — are enforced by **every** port's loader and encoded as **shared conformance error fixtures** (`error-enum-empty-values`, `error-enum-non-identifier-member`, `error-enum-duplicate-member`), each expecting `ERR_BAD_ATTR_VALUE`. Plus the universal `@values`-missing fixture → `ERR_MISSING_REQUIRED_ATTR` (free from `required: true`). Every port must pass all of these; no known-gap entries. (Java enforces attr constraints via a fluent `registerType` API — if per-element regex validation isn't expressible there declaratively, add a custom validator; escalate if it's more than a small change.)
...
- **Validation contract (revised during execution):** the `@values` rules (non-empty, identifier-safe members, no duplicates) are a **cross-language contract** enforced by every port's loader and encoded as shared conformance error fixtures (`error-enum-empty-values`, `error-enum-non-identifier-member`, `error-enum-duplicate-member`, all → `ERR_BAD_ATTR_VALUE`). Each port unit (TS Phase 5, C# Task 4.7, Java Phase 6, Python Phase 7) implements the checks so the shared fixtures pass everywhere. (Supersedes the earlier TS+C#-unit-tests-only plan — see Scope note.)
