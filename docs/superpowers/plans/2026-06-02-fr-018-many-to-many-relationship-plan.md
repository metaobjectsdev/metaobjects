# FR-018 Many-to-Many Relationship — Implementation Plan
...
**Coordination:** FR-018 changes the relationship vocabulary in the SP-G registry canonical. Land Phase 1 (vocabulary) BEFORE SP-G merges, or as a coordinated canonical update. FR-018 supersedes the SP-G Java-reconciliation join-vocab step.
...
- [ ] **Step 7 — Review + simplify gate. Commit.** `feat(metadata): FR-018 Unit1 — TS M:N slim vocabulary (through/sourceRefField/symmetric) + derivation + validation + fixtures`
...
- [ ] **Step 4 — Review + simplify gate. Commit.** `feat(metadata): FR-018 Unit2 — C# M:N slim vocabulary + validation (matches shared fixtures)`
...
- [ ] **Step 4 — Review + simplify gate. Commit.** `feat(metadata): FR-018 Unit3 — Java M:N slim vocabulary + validation`
...
- [ ] **Step 3 — Review + simplify gate. Commit.** `feat(metadata): FR-018 Unit4 — Python + Kotlin M:N slim vocabulary + validation`
...
- [ ] **Step 4 — Review + simplify gate. Commit.** `feat(runtime-ts): FR-018 Unit5 — M:N persistence fixtures + reference-derived resolver (hetero/self-join/symmetric)`
...
- [ ] **Unit 6 — Java OMDB.** Resolver in OMDB; persistence-conformance M:N green (Java). Review + simplify. Commit `feat(omdb): FR-018 Unit6 — Java runtime M:N resolver`
- [ ] **Unit 7 — Kotlin Exposed.** Resolver via Exposed; `integration-tests-kotlin` M:N green. Review + simplify. Commit `feat(codegen-kotlin): FR-018 Unit7 — Kotlin runtime M:N resolver`
- [ ] **Unit 8 — Python ObjectManager.** Resolver in ObjectManager; persistence-conformance M:N green (Python). Review + simplify. Commit `feat(python): FR-018 Unit8 — Python runtime M:N resolver`
- [ ] **Unit 9 — C# EF.** Resolver via EF; persistence-conformance M:N green (C#). Review + simplify. Commit `feat(csharp): FR-018 Unit9 — C# runtime M:N resolver`
...
- [ ] **Step 3 — Review + simplify gate. Commit.** `feat(codegen-ts): FR-018 Unit10 — M:N codegen (Drizzle m2m + nav + REST) + api-contract/codegen fixtures`
...
- [ ] **Unit 11 — C# codegen.** EF `UsingEntity<Through>(...)` + navigation + minimal-API traversal. Corpora green. Review + simplify. Commit `feat(csharp-codegen): FR-018 Unit11 — C# M:N codegen`
- [ ] **Unit 12 — Java codegen-spring.** Navigation + repository join + controller traversal + DTO (+ JPA `@ManyToMany`/`@JoinTable` where applicable). Corpora green. Review + simplify. Commit `feat(codegen-spring): FR-018 Unit12 — Java M:N codegen`
- [ ] **Unit 13 — Kotlin codegen.** Exposed many-to-many + controller. Corpora green. Review + simplify. Commit `feat(codegen-kotlin): FR-018 Unit13 — Kotlin M:N codegen`
- [ ] **Unit 14 — Python codegen.** SQLAlchemy `secondary=<junction>` + Pydantic nested + FastAPI traversal. Corpora green. Review + simplify. Commit `feat(python-codegen): FR-018 Unit14 — Python M:N codegen`
...
- [ ] **Step 3 — Review + simplify gate. Commit.** `feat(docs): FR-018 Unit15 — M:N documentation in shared docs engine (Tier 2, ADR-0020)`
...
- [ ] **Step 4 — Final review + merge.** Simplifier + final reviewer over the whole FR-018 diff (focus: vocabulary identical cross-port; derivation correct; three modes resolve correctly everywhere; codegen idiomatic + FR-007-parity; REST both lanes; no legacy vocab remains). Merge forward (integrate-before-merge).
...
- **Coordinate the canonical with SP-G.** Phase 1 Unit 1 changes `expected-registry.json`; if SP-G hasn't merged, land this first or update both together. FR-018 Unit 3 IS the relationship slice of the SP-G Java reconciliation.
