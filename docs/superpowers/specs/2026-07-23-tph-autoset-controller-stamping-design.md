...
- C#/TS are expected green but unproven until the gate runs; either could move to "fix."
- The Java/Kotlin/Python generated-lane harnesses use in-memory repositories — the direct-insert seed must bypass any repo-level stamping so the sentinel is genuinely old (a repo that stamps on insert would defeat the gate — a hollow-gate trap the vanilla rollout already navigated).
- `subtypeSettableFields` (and its Java/Python analogues) are shared with the `<Sub>Validation`/`<Sub>Dto` shape; excluding `@autoSet` changes that annotated shape — confirm no other consumer depends on the autoSet column being present in the settable set.
