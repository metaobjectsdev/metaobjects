...
1. **Write-verb shape:** a new `op: insert`/`roundtrip` in the persistence corpus (insert-via-runtime → read-back → assert), vs extending existing scenarios. Leaning: a dedicated round-trip scenario type so every subtype is covered uniformly.
2. **Bare `field.object` (no `@objectRef`):** map to jsonb-of-map (a generic structured column) vs require `@objectRef` + error clearly. Leaning: error clearly (a bare object with no shape is underspecified) — but confirm against existing usage.
3. **`field.class` cut confirmation:** the audit found zero consumers; confirm no adopter/runtime path (ADR-0001 `@object` binding) still needs it before removing.
