...
The C# loader + conformance shipped (final commit `463baef`, full corpus green, plan at `docs/superpowers/plans/2026-05-19-csharp-conformance-port.md`). The next natural step is RDB persistence integration. The user is also evaluating MetaObjects for a real-world C#/EF Core project currently in pain over EF Migrations — and wants the C# persistence work to **solve for that adopter specifically**, not just hit feature parity with the TS runtime.

This session explored the design space; we couldn't reach final approval because the next decisions depend on what the actual adopter codebase looks like. This doc preserves the state so a future session on the adopter's machine can resume cleanly.
...
Full survey of the legacy Java `ObjectManagerDB` module in a sibling project. Architecturally older but with some good ideas worth preserving:
...
## What the C# adopter is up against (validated community pain, not just one team)
...
5. **Slicing PIVOT for the C# adopter:** **migrations are Slice 1, runtime is later.** The adopter already has EF for queries; what they need is the migration replacement. The runtime ObjectManager is the longer-term arc, not the immediate-value wedge.

---

## Open questions — resolve on the adopter's-machine session

These need answers grounded in the actual adopter codebase, not abstract preference.

1. **Adoption path** — reverse-engineer metadata from the adopter's live DB / hand-write metadata to match / both? (Asked this session; user redirected to "let's look at the actual project first.")
...
6. **EF entity-class story.** Once metadata becomes canonical, does the adopter (a) keep hand-edited EF entity classes drifting from metadata, (b) replace them with MetaObjects-codegen'd EF classes, or (c) keep their EF classes but treat them as the source of truth and have MetaObjects reverse-engineer metadata from the EF model annotations? Each has implications for which side wins on a drift.
7. **Migration history.** What's already in the adopter's `Migrations/` folder? Discard? Preserve as historical? MetaObjects starts emitting NEW scripts from "current state" forward, and the historical EF migrations stay as a static record of how we got there.
8. **Test / CI fit.** How does the adopter run migrations today? `dotnet ef database update` in CI? A startup hook? A separate deploy job? MetaObjects needs to slot into the same lifecycle.

---

## Artifacts to look at next session

- **The adopter's C# project root** — directory structure, csproj files, target frameworks.
...
## Recommended provisional direction (validate against adopter reality)
...
MetaObjects emits raw SQL. Users apply with DbUp / Grate / hand-rolled. We don't own the application — we own the **authoring**. Out tracking table name (`__metaobjects_migrations`) or interop with DbUp's table (`SchemaVersions`) — decision deferred until we know the adopter's actual lifecycle.

### What's in vs out for v1 (adopter-driven)

**In:**
- `meta init --from-db` for SqlServer **and** Postgres (the adopter likely on SqlServer; cover both for cross-platform parity).
...
5. **EF coexistence story.** The adopter will still have EF. The MetaObjects tool MUST NOT touch EF's `__EFMigrationsHistory` table or step on EF's view of "current schema." Adoption is additive at first, replacement later.
...
7. **Multi-environment.** The adopter likely has dev / staging / prod. The same script must apply identically across environments (no `if ENV=prod` branches in MetaObjects-emitted SQL — that's policy, not schema).
...
2. **Walk the adopter codebase per the "artifacts to look at" list above.** Don't ask the user questions you can answer by reading code first.
3. **Answer the eight "open questions" against what you find.** Update this doc inline with the concrete adopter-grounded answers (commit the update).
...
## The first question to re-ask once you've looked at the adopter
...
The answer is highly likely (C) for any real codebase, but DO look first — if the adopter schema is genuinely clean and metadata-shaped (uniform naming, simple FKs, no triggers/computed columns/checks), (A) might suffice and slice 1 gets smaller.
...
- **TS migration reference:** `typescript/packages/migrate-ts/` (worth a survey next session; we didn't read it this session because we pivoted to adopter focus before exploring it).
- **Legacy Java reference:** the `ObjectManagerDB` module in a sibling project.
- **Adopter codebase:** TBD — to be located on the adopter's-machine session.
