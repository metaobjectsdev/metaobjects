...
- **C#:** host the generated minimal-API routes + generated `AppDbContext` over HTTP against **Testcontainers Postgres** — the fully-generated C# stack, faithful like SP-B's full-stack TS lane. (Implementation note: the emitted routes have no `Program` entry-point assembly for `WebApplicationFactory<TEntryPoint>`, so the lane hosts a real Kestrel `WebApplication` on a loopback port + a real `HttpClient` — *more* faithful than the in-process TestServer originally proposed, since it exercises the real socket + pipeline.)
