...
Output lands under `--output-dir` (default `Generated`). The runner overwrites only
files bearing this header; it refuses to touch files without it. Locate the output
directory from how the team invokes `dotnet meta gen` (or a CI script) — the arg is
explicit, there is no config file equivalent on the C# side.
...
The `dotnet meta agent-docs` command (re)scaffolds the slim always-on Markdown +
the `metaobjects-*` skills into the project using the agent-context bundled inside
the .NET tool — no Node `meta` is required for this on the C# side.
