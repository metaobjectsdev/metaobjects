...
| `namesFile()` | `<Entity>.names.ts` — `export const <Entity>Names` carrying the object's physical database names (`kind`/`name`/`schema`/`readOnly` + `fields.<field>.{name,column}`). Emitted for every object with a declared or inherited primary source, PLUS a fragment for any abstract base such an object extends (columns only, no `name` — it has no table). An artifact whose object extends another one spreads the parent's rather than restating it: `...BaseEntityNames.fields` for an inherited column set, `...AuthNames` for a TPH subtype, which shares its base's table. When this generator is in the run, `entityFile()` and `routesFile()` reference the constants instead of embedding the literals; drop it from the suite and they fall back to literals. |
