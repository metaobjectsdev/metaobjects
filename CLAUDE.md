...
**TypeScript reference implementation** is **published to npm at `0.14.0`** (all 13 `@metaobjectsdev/*` publish candidates on the `latest` tag, full lockstep). C# at `0.14.0` (NuGet); Python at `0.14.0` (PyPI); Java / Kotlin at `7.6.0` (Maven Central). 2500+ tests passing across the workspace. (The `0.14.0` / `7.6.0` line is a **coordinated minor with breaking changes**: `verify` is now **strict-by-default + `--lax`/`-Dmeta.lax`** across all four CLI ports [ADR-0023], and a **jsonb open-bag** [`field.string @dbColumnType:jsonb`] is exposed as a **parsed JSON value** [string→object] at the API boundary in all five ports — both gated by a new cross-port api-contract corpus; plus **`formFile` nested value-object sub-forms**.)
...
**Current release line.** `0.14.x` (npm / NuGet / PyPI) · `7.6.x` (Maven Central). The `0.14.0` / `7.6.0` release is a coordinated **minor with breaking changes**: (1) `verify` is **strict-by-default** with a `--lax`/`-Dmeta.lax` escape across all four CLI ports — an unregistered/typo'd `@attr` now fails the gate (ADR-0023); (2) a **jsonb open-bag** (`field.string @dbColumnType:jsonb`) is exposed as a **parsed JSON value** rather than a string at the API boundary in all five ports; (3) **`formFile`** emits nested value-object sub-forms. Both (1) and (2) are gated by new cross-port conformance corpora. See `CHANGELOG.md` for the per-version detail. GA promotion is the next release move.
