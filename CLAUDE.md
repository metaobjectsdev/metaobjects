...
_Last refreshed 2026-08-08._

**TypeScript reference implementation** is **published to npm at `0.21.0`** (14 `@metaobjectsdev/*` publish candidates on the `latest` tag, full lockstep; the two `angular` packages are on their own `0.6.x` line). C# at `0.21.0` (NuGet); Python at `0.21.0` (PyPI); Java / Kotlin at `7.21.0` (Maven Central). **All four registries share the same `minor.patch`, and stay locked in lockstep going forward** — the `0.20.15` cut moved Maven `7.20.12`→`7.20.15` to close the last per-registry drift.
