...
**TypeScript reference implementation** is **published to npm at `0.15.5`** (all 13 `@metaobjectsdev/*` publish candidates on the `latest` tag, full lockstep). C# at `0.15.4` (NuGet); Python at `0.15.6` (PyPI — stays a patch ahead on the same line); Java / Kotlin at `7.7.3` (Maven Central). The `0.15.5` / PyPI `0.15.6` patch ships two consumer-provider CLI fixes: **npm `0.15.5`** — offline `meta migrate` now threads `metaobjects.config.ts` providers, so a config-registered custom subtype resolves on offline migration like it already did for `meta gen` (#157); **PyPI `0.15.6`** — a new `metaobjects gen|verify|docs --provider module:symbol` hook lets the standalone Python CLI load an app's custom provider (#158, parity with TS `config.providers`; Java/Kotlin already discover providers via ServiceLoader on the project classpath, and C#'s `dotnet meta` hook is tracked as future work). C#/Java/Kotlin needed no code change and stay at their current versions. The `0.15.4` line fixes cross-package same-name root nodes being wrongly merged by the TS/C#/Python loaders (#155). 2500+ tests passing across the workspace. (The `0.15.1` / `7.7.1` line shipped the **`index.*` type + `identity.secondary` key-purity** [ADR-0040]: `identity.secondary` is now a *unique* alternate key with `@unique` **removed** — a legacy `@unique` fails load with `ERR_UNKNOWN_ATTR` — and a new **`index.lookup`** subtype carries *non-unique* retrieval [`@fields` required; `@using`/`@expr`/`@where`/`@orders` are RDB-physical escapes], cross-port conformance-gated; **breaking**, migration in `docs/features/migrations/identity-secondary-to-index-lookup.md`. This is on top of the `0.15.0` metamodel-1.0 vocabulary program + the ADR-0039 own-accessor fix.)
