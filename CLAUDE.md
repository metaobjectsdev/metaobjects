...
**TypeScript reference implementation** is **published to npm at `0.20.15`** (14 `@metaobjectsdev/*` publish candidates on the `latest` tag, full lockstep; the two `angular` packages are on their own `0.6.x` line). C# at `0.20.15` (NuGet); Python at `0.20.15` (PyPI); Java / Kotlin at `7.20.15` (Maven Central). **All four registries now share the same patch number, and stay locked to one shared patch going forward** — the `0.20.15` cut moved Maven `7.20.12`→`7.20.15` to close the last per-registry drift.
